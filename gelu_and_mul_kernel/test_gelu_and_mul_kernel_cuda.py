...
def save_inputs_outputs(path, bs=4, hidden_dim=64, quantize=None):
    ...
    hidden_states = torch.randn((bs, hidden_dim * 2), dtype=torch.float32, device="cuda")
...
def run_and_compare(path, rtol: float = 1e-3, atol: float = 1e-3):
    ...
    expected_out_hidden_states = data['out_hidden_states'].cuda()
    expected_out_scales = data['out_scales'].cuda() if data['out_scales'] is not None else None

    assert torch.allclose(out_hidden_states, expected_out_hidden_states, rtol=rtol, atol=atol), \
        f"Output hidden states mismatch: {out_hidden_states} vs {expected_out_hidden_states}"
    
    if expected_out_scales is not None:
        assert torch.allclose(out_scales, expected_out_scales, rtol=rtol, atol=atol), \
            f"Output scales mismatch: {out_scales} vs {expected_out_scales}"
