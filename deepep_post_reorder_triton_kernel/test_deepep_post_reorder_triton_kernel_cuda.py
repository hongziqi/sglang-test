...
def run_and_compare(path: str, rtol=1e-3, atol=1e-3, BLOCK_SIZE: int = 64):
    ...
    down_output = data["down_output"].cuda()
    src2dst = data["src2dst"].cuda()
    topk_ids = data["topk_ids"].cuda()
    topk_weights = data["topk_weights"].cuda()
...
if __name__ == "__main__":
    # 保存输入输出
    save_inputs_outputs("deepep_post_reorder_cuda_output.pt")

    # 加载输入并比较重复输入输出精度
    run_and_compare("deepep_post_reorder_cuda_output.pt")
