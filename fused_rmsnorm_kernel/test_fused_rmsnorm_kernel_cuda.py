...
def save_inputs_outputs(path, batch_size=4, hidden_dim=128, eps=1e-6):
    ...
    weight_tensor = torch.randn(hidden_dim, dtype=torch.float32, device='cuda')
...
def run_and_compare(path, rtol: float = 1e-3, atol: float = 1e-3):
    ...
    weight_tensor = data['weight_tensor'].cuda()
    eps = data['eps']
    ...
    assert torch.allclose(output_tensor, expected_output, rtol=rtol, atol=atol), \
        f"Output mismatch: {output_tensor} vs {expected_output}"
