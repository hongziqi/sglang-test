...
def save_inputs_outputs(path, batch_size=4, hidden_dim=128, eps=1e-6):
    ...
    input_tensor = torch.randn(batch_size, hidden_dim, dtype=torch.float32, device="cuda")
    residual = torch.randn(batch_size, hidden_dim, dtype=torch.float32, device="cuda")
    weight1 = torch.randn(hidden_dim, dtype=torch.float32, device="cuda")
    weight2 = torch.randn(hidden_dim, dtype=torch.float32, device="cuda")
...
def run_and_compare(path, rtol: float = 1e-3, atol: float = 1e-3):
    ...
    residual_tensor = data["residual_tensor"].cuda()
    weight1_tensor = data["weight1_tensor"].cuda()
    weight2_tensor = data["weight2_tensor"].cuda()
    eps = data["eps"]
    ...
    expected_mid = data["mid_tensor"].cuda()

    assert torch.allclose(output_tensor, expected_output, rtol=rtol, atol=atol), \
        "Output tensor does not match expected output."
    assert torch.allclose(mid_tensor, expected_mid, rtol=rtol, atol=atol), \
        "Mid tensor does not match expected mid tensor."
