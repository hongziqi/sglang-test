...
def save_inputs_outputs(path, n_elements=1024, softcap_const=0.5):
    ...
    input_tensor = torch.randn(n_elements, dtype=torch.float32, device="npu")
...
def run_and_compare(path, rtol=1e-3, atol=1e-3):
    ...
    check_accuracy(output.cpu(), expected_output.cpu())


if __name__ == "__main__":
    # 编译测试
    path = "fused_softcap_kernel_npu_output.pt"
    ...
    # Max diff at (tensor(481),): test=-0.4709279537200928, ref=-0.4709278643131256, abs=8.940696716308594e-08, rel=1.898523720456069e-07
