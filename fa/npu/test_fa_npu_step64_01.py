...
test_results = []  # 全局结果存储
valid_fields = ["Z", "H", "N_CTX", "HEAD_DIM", "causal", "dtype", "BM", "BN", "step", "Testcase Name", "sparse mode"]
...
def pytest_generate_tests(metafunc):
    ...
    if 'test_case' in metafunc.fixturenames:
        # 生成测试用例数据
        paths = {
            "step64": os.path.join(TEST_DATA_DIR, "FlashAttentionScore_step64_case_d64_Result_01.xls"),
        ...
        # 非测试文件的测试案例
        # test_cases = [
        #     [1, 24, 9792, 64, False, torch.float16, 64, 64, "step64", "FlashAttentionScore_BNSD_0153", 0],
...
def test_op_fwd(test_case:  Union[Dict[str, Any], List[Any]]):
    ...
    # Z, H, N_CTX, HEAD_DIM, causal, dtype, BM, BN, step, test_name, sparse_mode = test_case
    Z, H, N_CTX, HEAD_DIM, causal, dtype, BM, BN, step, test_name, sparse_mode = [test_case[k] for k in valid_fields]
    ...
    try:
        ...
        # # 性能测试
        # kernel_avg_time = do_bench(profiling_forward_fn, keep_res=False)
